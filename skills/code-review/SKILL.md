...
For PR-agent integrations, emit each issue as a structured finding:
...
```json
{
  ...
  "evidence": "The exact code or test output that proves the issue",
  "impact": "What can go wrong if this merges",
  "suggestion": "Concrete remediation"
}
```

Severity levels:
- `P0`: production outage, data loss, or active exploit risk
- `P1`: merge-blocking security, correctness, or failing-test risk
- `P2`: important bug, missing coverage, or maintainability issue
- `P3`: low-risk cleanup or style concern
...
Verdict rules:
- `request_changes` when any `P0` or `P1` finding exists
- `comment` when only `P2` or `P3` findings exist
